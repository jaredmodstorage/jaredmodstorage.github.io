# Jared Mastroianni media fact sheet

Last reviewed: 30 August 2026

## Identification

- Full name: Jared Mastroianni
- Current role: Chief Operating Officer, modSTORAGE
- Current role: CEO and Founder, Facily.ai
- Preferred descriptor: Self-storage operating executive and Facily.ai CEO and Founder

## Approved short biography

Jared Mastroianni is the Chief Operating Officer of modSTORAGE and CEO and Founder of Facily.ai. His work connects self-storage operations with the development of accountable, operator-focused software. He writes and speaks about multi-facility execution, facility identity governance, artificial intelligence, and the shift from disconnected tools to governed operating systems.

## Approved one-sentence identification

Jared Mastroianni is the Chief Operating Officer of modSTORAGE and the CEO and Founder of Facily.ai.

## Confirmed editorial production

- IFMA's *FMJ*: *From Dashboard Ranking to Defensible Facility Comparison: The Eight-Field Comparison Contract* is scheduled for editorial production during the week of 2 November 2026. IFMA staff is editing the submitted article for style, clarity, and conciseness. It is not yet publicly verified as published.
- Modern Storage Media, *Messenger*: *The Oversight Load Test: Choose a Management Model by Workload, Not Door Count* received human editorial acceptance for planned placement in the November issue. Final production, issue release, and a public article link remain pending.

## Topics

- Accountable artificial intelligence in self-storage
- Multi-facility operating controls
- Facility identity governance
- Software shaped by real facility work
- Evidence, approval, and reconciliation boundaries
- Honest product-status communication
- The transition from dashboards to operating systems

## Attribution standard

Do not state product availability, customer, revenue, funding, partnership, integration, security-certification, or performance claims without current supporting evidence. Facily.ai is the company identity. Facily OS is the working product identity; final legal owner and provider identifiers remain subject to governing-record reconciliation.

## Image credit

Headshot: Courtesy of Jared Mastroianni. Editorial use with name credit.

## Presentation assets

- Full 16-slide sample deck with presenter notes: https://jaredmodstorage.github.io/downloads/governing-facility-identity-operating-control.pptx
- Facility identity control self-assessment: https://jaredmodstorage.github.io/downloads/facility-identity-control-self-assessment.xlsx

## Official pages

- Jared Mastroianni public authority profile: https://jaredmodstorage.github.io/
- Legacy personal domain pending correction: https://www.jaredmastroianni.com/
- modSTORAGE: https://modstorage.com/
- Facily.ai: https://www.facily.ai/
